...
class Detector:
    ...
    def __init__(self, datapath="data/dataset.csv", modelpath="model",mode=1):
	    ...
	    #print self.testingData.select("features").collect()
    ...
    def predict(self): 
	#print self.predictingData.select('*').collect()
        predictions = self.model.transform(self.predictingData)
	df= predictions.select('prediction').collect()
	#df= predictions.select("prediction")[0].asDict().values()
	#df= predictions.select("prediction")
